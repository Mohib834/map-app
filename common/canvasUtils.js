...
var draw = function (ctx, text, fontObject) {
  ...
  // Draw text
  console.log("options.textcolours", options.textcolours);
  var nTextColours = options.textcolours.length;
  if(nTextColours == 1) {
    var col = options.textcolours[0];
    // 1 colour so draw everything together
    var fontPath = fontObject.getPath(text, xPos, yPos, fontSize);
    fontPath.fill = 'rgb('+col[0]+', '+col[1]+', '+col[2]+')';
    fontPath.draw(ctx);
  }
  else {

    // more than 1 colour so draw each letter separately
    var glyphs = textMetrics.glyphs;
    var glyphPositions = textMetrics.glyphPositions;
    var textcolix = 0;

    for (var i = 0; i < glyphs.length; i++) {
      var col = options.textcolours[textcolix];
      textcolix = (textcolix + 1) % nTextColours;
      var path = glyphs[i].getPath(xPos + glyphPositions[i], yPos, fontSize);
      path.fill = 'rgb(' + col[0] + ', ' + col[1] + ', ' + col[2] + ')';
...
function fillText(canvas, text, x,y, font, fontSize, textAlign = 'center', textBaseline = 'center', textcolours=[[0,0,0]])
{
  let rect = {
    x: x,
    y: y,
