...
var MarcTooltips=(function(){
	...
	var _relocateTooltip=function(tooltip, position, align){
		...
		// dh hack
		//rect.top -= 50;
		//console.log("_relocateTooltip rect:", rect);
		...
		if(position==='up'){
			...
			if(align==='top'){
				...
				let OFFSET_HACK = -3; // align to the middle
				tooltip.style.top=parseInt(OFFSET_HACK + rect.top+top-parseInt((tooltip.offsetHeight-tooltip.attachedTo.offsetHeight)/2))+'px';
			}
		}
	...
	return{
		add:function(els,str,props){
			//console.log("marctooltips add:", els,str);
      ...
      tooltip.style.textTransform = "capitalize";
        ...
        tooltip.className=tooltip.className.replace(' visible','');
          ...
          //console.log("add mouseover event", els2[i]);

          addEvent(els2[i], 'mouseenter', showTooltip);
					addEvent(els2[i], 'mouseleave', hideTooltip);
				}
			}
