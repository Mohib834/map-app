	...
	var methods = {
		init: function (options) {
			...
			return this.each(function (e) {
				...
				//		console.log("jquery.itour.js localStore", hData);
						return hData.userOpt.localStore;
					};
                  ...
                  hData.opt.steps[hData.v.startIndex].contentPosition == 'centerAndFocus'){
									contentPos = ['c','c','c'];
									left = '50%';
									top = '50%';
								}else{
            ...
            if(hData.v.overlays){
						...
						hData.v.hContPos.stop(true).fadeOut(100,function(){
							...
							if(hWrap.data().itour_go){
								changeDelay = 1;	
								localStore().removeItem('itour_go');
												...
												if(hData.v.targetEl.is(':visible')){
													...
													var myFunc = function(){
														...
														if (typeof(Storage) !== "undefined"){
															...
															localStore().setItem('itour_data-'+hData.opt.tourID,JSON.stringify(itour_data));
														}
                            ...
                            if(!hData.opt.steps[hData.v.startIndex].event){
														...
														if(hData.opt.steps[hData.v.startIndex].event == 'next'){
															if(!$(hNext).length){
																hNext = $('<span class="hBtn hNext">'+hData.opt.steps[hData.v.startIndex].nextText+'</span>').prependTo(hData.v.hContFooter);
															}
														}else{
					...
					var storageDetect = function(){
						...
						setTimeout(function(){
							introMess.remove();
								if (typeof(Storage) !== 'undefined'){
									console.log("hWrap",hWrap);
									console.log("localStore",localStore);

									if(localStore().getItem('itour_data-'+hData.opt.tourID)){
										var nowLoc = location.href;
										var itour_data = $.parseJSON(localStore().getItem('itour_data-'+hData.opt.tourID));
										if(itour_data.tourId == hData.v.tourid){
											if(hWrap.data().itour_go){
												...
												if(hData.opt.tourContinue){
													hData.v.startDialog = $('<div>')
														...
														.html('<div class="hContHeader">'+hData.opt.lang.contDialogTitle+'</div><div class="hContBody">'+hData.opt.lang.contDialogContent+'</div><div class="hContFooter" style="text-align:right"><span class="hBtn continueBtn">'+hData.opt.lang.contDialogBtnContinue+'</span><span class="hBtn startOverBtn">'+hData.opt.lang.contDialogBtnBegin+'</span></div>')
														.appendTo('body');
											...
											if(nowLoc.search(escapeRe(targetLoc)) === -1 && targetLoc.search(escapeRe(nowLoc)) === -1 && nowLoc !== targetLoc){
												...
												localStore().setItem('itour_go',JSON.stringify(itour_go));
												...
												var itour_data = {};
												if(localStore().getItem('itour_data-'+hData.opt.tourID)){
													itour_data = $.parseJSON(localStore().getItem('itour_data-'+hData.opt.tourID));
												}
												$.extend(itour_data, {
													tourId:hData.v.tourid,
						...
						if (hData.opt.introShow){
							if(hWrap.data().itour_go){
								...
								hData.v.introDialog = $('<div>')
									...
									.html('<div class="hContImage"></div><div class="hContHeader">'+hData.opt.lang.introTitle+'</div><div class="hContBody">'+hData.opt.lang.introContent+'</div><div class="hContFooter" style="text-align:right"><span class="hBtn hStartBtn">'+hData.opt.lang.introDialogBtnStart+'</span><span class="hBtn hAbortBtn hClose">'+hData.opt.lang.introDialogBtnCancel+'</span></div>')
									.appendTo('body');
	...
	$(window).on('load',function(){
		...
		if (typeof(Storage) !== 'undefined'){
			...
			if(localStorage){
				...
				if(localStore().getItem('itour_go')){
					...
					var itour_go = $.parseJSON(localStore().getItem('itour_go'));
