...
/*jslint white: true, vars: true, nomen: true, newcap: true*/
...
function MedianCut() {
    ...
    function Box() {
        ...
        // get the median
        function median(){
            var index = this.median_pos();
            return data[index];
        }

        // mean
        function average(startIndex = null, endIndex = null) {
    ...
    function getColourSegmentsMean(boxes)
    {
        var segments = [];
        for(var i = boxes.length - 1; i >= 0; i -= 1 ) {
            segments.push( {
                "colour": boxes[i].average(),
                "weight": boxes[i].count()
            } );
        }
        return segments;
    }
    ...
    function get_dynamic_size_boxes( _threshold ) {
    ...
    function get_fixed_size_boxes( _number ) {
    ...
    return {
        ...
        // These are exposed (public) functions
        init                     : init,
        get_fixed_size_boxes   : get_fixed_size_boxes,
        get_dynamic_size_boxes : get_dynamic_size_boxes,
        getColourSegmentsMean: getColourSegmentsMean
    };
}
